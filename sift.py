...
class SIFT():
    ...
    def get_maxima_minima_single_octave(self, i):
        ...
        for scale in range(1, self.num_intervals - 2):
            ...
            for row in range(1, row_bound-1):
                for col in range(1, col_bound-1):
                    ...
                    hessian_matrix = self.get_hessian_matrix(row, col, prev_scale, current_scale, next_scale)
                    # check local maxima and minima
                    is_extrema = self.check_extrema(current_scale[row][col], neighbours)
                    # if pixel is local extrema and it's hessian is invertible then proceed
                    if is_extrema == True and np.linalg.det(hessian_matrix) != 0:
                        # generate a keypoint object
                        subpixel_value, delta_X = self.get_subpixel_maxima_minima(row, col, prev_scale, current_scale, next_scale, hessian_matrix)
                        check_for_edge = self.get_edge_check(hessian_matrix)
                        if subpixel_value > 0.03 and check_for_edge == False:
                            ...
                            # append to the global keypoints list to be given as output
     ...
     # return the list of 26 neighbours of a pixel in current, previous and next scale
    ...
    # helper function for get_neighbours function to calculate direction to find neighbours
    ...
    def get_subpixel_maxima_minima(self, row, col, prev_scale, current_scale, next_scale, hessian_matrix):
        # multiply negative of inverse of hessian matrix with gradient matrix
        # formulae from reference https://www.ipol.im/pub/art/2014/82/article_lr.pdf
        ...
        sub_pixel_value = current_scale[row, col] + ((1 / 2) * 
                                                     np.matmul(gradient_matrix.transpose(), delta_X)) 
    ...
    # calculate hessian matrix
    def get_hessian_matrix(self, row, col, prev_scale, current_scale, next_scale):
        # formulae from reference https://www.ipol.im/pub/art/2014/82/article_lr.pdf
        dxx = (current_scale[row, col+1] + current_scale[row, col-1] - 2 * current_scale[row, col]) 
        dyy = (current_scale[row+1, col] + current_scale[row-1, col] - 2 * current_scale[row, col])
        dscale_scale = (next_scale[row, col] + prev_scale[row, col] - 2 * current_scale[row, col])
        dxy = (current_scale[row+1, col+1] - current_scale[row-1, col+1] - current_scale[row+1, col-1] 
               + current_scale[row-1, col-1]) / 4 
        dx_scale = (next_scale[row, col+1] - prev_scale[row, col+1] - next_scale[row, col-1]
                    + prev_scale[row, col-1]) / 4
        dy_scale = (next_scale[row+1, col] - prev_scale[row+1, col] - next_scale[row-1, col]
    ...
    def get_gradient_matrix(self, row, col, prev_scale, current_scale, next_scale):
        # formulae from reference https://www.ipol.im/pub/art/2014/82/article_lr.pdf
...
sift = SIFT(image)
kp = sift.detect()
img1=cv2.drawKeypoints(gray1, kp, np.array([]))
